...
from tools21cm.noise_model import noise_cube_coeval
from . import HERAnoise as hn
from . import core
from . import pyMFs as MFs
    ...
    def _read_data(self):
        ...
        for fl in self.datafile:
            if not path.exists(fl):
                ...
                data.append(np.load(fl, allow_pickle=True).item())
        ...
        if self.noisefile:
            ...
            for fl in self.noisefile:
                if not path.exists(fl):
                    ...
                    noise.append(np.load(fl, allow_pickle=True).item())
...
class LikelihoodMFs(LikelihoodBaseFile):
    required_cores = (core.CoreCoevalModule,)
    def __init__(
        self,
        steps=0.1,
    ...
    def _check_data_format(self):
        for i, d in enumerate(self.data):
            if "v0" not in d or "v1" not in d or "v2" not in d or "v3" not in d:
                raise ValueError(
                    f"datafile #{i+1} of {len(self.datafile)} has the wrong format."
                )
    ...
    def computeLikelihood(self,model):
        lnl=0
        for i, (m, pd) in enumerate(zip(model, self.data)):
            #mask0=np.logical_not(np.abs(pd["v0"])<=0.01*np.max(np.abs(pd["v0"])))
            #mask1=np.logical_not(np.abs(pd["v1"])<=0.01*np.max(np.abs(pd["v1"])))
            #mask2=np.logical_not(np.abs(pd["v2"])<=0.01*np.max(np.abs(pd["v1"])))
            #mask3=np.logical_not(np.abs(pd["v3"])<=0.01*np.max(np.abs(pd["v1"])))
            moduncert0 = self.model_uncertainty[i]["v0"]*10
            moduncert1 = self.model_uncertainty[i]["v1"]*self.likelihood
            moduncert2 = self.model_uncertainty[i]["v2"]*self.likelihood
            moduncert3 = self.model_uncertainty[i]["v3"]*self.likelihood
    ...
    def reduce_data(self, ctx):
        ...
        i=0
        for bt in brightness_temp:
            v0,v1,v2,v3=self.compute_MFs(bt,self.user_params.BOX_LEN,self.user_params.HII_DIM,z[i],max_sig=self.max_sig,min_sig=self.min_sig,steps=self.steps,is_smooth=self.is_smooth,is_noise=self.is_noise,uvmap=self.uvmap[i],nant=self.nant,obs_hours=self.obs_hours)
            data.append({"v0": v0,"v1": v1,"v2": v2,"v3": v3})
            i+=1
    ...
    def store(self, model, storage):
        ...
        storage backend.
        """
        # add the MFs to the written data
        for i, m in enumerate(model):
            storage.update({k + "_z%s" % self.redshift[i]: v for k, v in m.items()})


class LikelihoodMFs_hera(LikelihoodBaseFile):
    required_cores = (core.CoreCoevalModule,)
    def __init__(
        self,
        steps=0.1,
    ...
    def compute_MFs(
        ...
        if is_smooth:
            data=t21c.smooth_coeval(brightness_temp,z,box_size_mpc=L,max_baseline=0.28)
        else:
            data=brightness_temp
        if is_noise:
            noise=t21c.smooth_coeval(hn.noise_cube_coeval_hera(ndim,z,obs_time=obs_hours,uv_map=uvmap,N_ant=nant,boxsize=L),z,box_size_mpc=L,max_baseline=0.28)
...
class LikelihoodMFs_Cov(LikelihoodBaseFile):
    required_cores = (core.CoreCoevalModule,)
    def __init__(
        self,
        steps=0.1,
    ...
    def computeLikelihood(self,model):
        ...
        for i, (m, pd) in enumerate(zip(model, self.data)):
            ...
            #mask2=np.logical_not(np.abs(pd["v2"])<=0.01*np.max(np.abs(pd["v1"])))
            #mask3=np.logical_not(np.abs(pd["v3"])<=0.01*np.max(np.abs(pd["v1"])))
            MFs_delta=np.concatenate((m["v1"],m["v2"],m["v3"])).reshape(183,1)-np.concatenate((pd["v1"],pd["v2"],pd["v3"])).reshape(183,1)

            lnl+=np.sum(np.matmul(MFs_delta,MFs_delta.T)*self.model_uncertainty[i])
            # TODO: if moduncert depends on model, not data, then it should appear
            #  as -0.5 log(sigma^2) term below.
            #lnl += -0.5 * np.sum(
                #(m["v0"] - pd["v0"]) ** 2
